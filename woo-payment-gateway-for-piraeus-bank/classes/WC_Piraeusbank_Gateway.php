...
class WC_Piraeusbank_Gateway extends WC_Payment_Gateway
{
    ...
    public function check_piraeusbank_response()
    {
        ...
        if ($this->redirect_page_id == -1 && $order !== null) {
            ...
            $redirect_url = add_query_arg(array('msg' => urlencode($pb_message['message']), 'type' => $pb_message['class']), ($this->redirect_page_id === "" || $this->redirect_page_id === 0) ? get_site_url() . "/" : get_permalink($this->redirect_page_id));
        }
    ...
    public function validate_fields()
    {
        if (is_checkout() && WC()->session->get('chosen_payment_method') === $this->id && $this->pb_cardholder_name === 'yes') {
            if (!isset($_POST['cardholder_name']) || trim($_POST['cardholder_name']) === '') {
                wc_add_notice(
                    __('Cardholder name is mandatory', self::PLUGIN_NAMESPACE),
                    'error'
                );
            }
        }
